...
public class OrderComplainService {
	...
	public void doUpdateOrderComplaint(Map<String, Object> filter) {
		...
		String userId = VariableUtils.typeCast(SessionVariable.getCurrentSessionVariable().getUser().get("id"),
				String.class); //
		Integer id = VariableUtils.typeCast(filter.get("id"), Integer.class); // 投诉订单编号
	...
	public int getOrderComplaintCount(Map<String, Object> filter) {
		...
		OrderComplaint orderComplaint = new OrderComplaint();
		// 当前地市
		orderComplaint.setAreaId(VariableUtils.typeCast(SessionVariable.getCurrentSessionVariable().getAreaId(),
				String.class));
		// 状态
