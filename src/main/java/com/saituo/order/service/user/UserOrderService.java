...
=======
	...
	public Map<String, String> doCreateUserOrder(Map<String, Object> filter) {
		...
		// 受理地市----------------------------------------------------如何获取???????????????????????????????????????????????????????????????????????????????????????
		String areaId = "189";// VariableUtils.typeCast(SessionVariable.getCurrentSessionVariable().getUser().get("company_id"),String.class);
		// 客户组别编码----------------------------------------------------如何获取?????????????????????????????????????????????????????????????????????????????????????
		String groupId = "888";// VariableUtils.typeCast(SessionVariable.getCurrentSessionVariable().getUser().get("company_id"),
								// String.class);
		// 地址编码
		Long addressId = 0l;
	...
	public void doUpdateProductOrderFee(Map<String, Object> filter) {
		...
		if (productOrderList != null && productOrderList.size() > 0) {
			...
			for (String productOrderString : productOrderList) {
				...
				//记录产品订单项的操作历史
				orderResult="修改订购价格：修改前："+VariableUtils.typeCast(prodcutString[2], String.class)+" 修改后："+VariableUtils.typeCast(prodcutString[1], String.class);
