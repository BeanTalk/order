/*
 * ! jQuery blockUI plugin Version 2.70.0-2014.11.23 Requires jQuery v1.7 or
 * later
 * 
 * Examples at: http://malsup.com/jquery/block/ Copyright (c) 2007-2013 M. Alsup
	...
	function setup($) {
		...
		$.growlUI = function(title, message, timeout, onClose) {
			...
			$m.mouseover(function() {
						...
						displayBlock.stop(); // cancel fadeout if it has
												// started
						displayBlock.fadeTo(300, 1); // make it easier to
														// read the message by
		...
		function install(el, opts) {
			...
			var expr = setExpr
					&& (!$.support.boxModel || $('object,embed', full
									? null
									: el).length > 0);
			if (ie6 || expr) {
